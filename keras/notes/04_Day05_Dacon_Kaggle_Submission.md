...
## 14. Kaggle Bike에서 앞으로 개선할 부분

현재 `index_col=0`으로 읽으면 `datetime`이 모델 feature에서 제외된다.

향후 feature engineering을 배우면 날짜 정보를 분해할 수 있다.
...
또한 target 분포와 대회 metric에 따라 log 변환을 사용할 수도 있다.
...
무조건 사용하는 것이 아니라 데이터와 평가 지표를 보고 결정한다.

---

## 15. 해외 논문 공부 방향

앞으로 모델이 복잡해지면 단순 코드 실행에서 끝내지 않고 다음 순서로 공부한다.
...
결측치 처리
↓
Dacon / Kaggle Submission
```

아직은 Dense 모델을 이용한 기초 회귀 문제를 다루는 단계다.
...
> **Pandas로 실제 CSV 데이터를 정리하고 Dense 회귀 모델을 학습한 뒤 MSE/RMSE/R²로 평가하고, Dacon/Kaggle의 test 데이터를 예측하여 submission.csv까지 만드는 전체 기초 pipeline을 학습했다.**
